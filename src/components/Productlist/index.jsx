...
export default function ProductList({ ligthAndDark }) {
  ...
  const handleSortOption = (props) => {
    setSortName(props.name)
    sortOptions.map(item => {
      item.current = false
    })
    props.current = true
    props.name === 'Price: Low to High' && data.sort((a, b) => a.price - b.price);
    props.name === 'Price: High to Low' && data.sort((a, b) => b.price - a.price);
    props.name === 'Newest' ? data = copyData.reverse() : data = copyData
  }
  return (
    <div>
      <div>
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
            <h1 className="text-4xl font-bold tracking-tight   -900">New Arrivals</h1>
            ...
            <div className="flex items-center">
              <Menu as="div" className="relative inline-block text-left">
                <div>
                  <MenuButton className="group inline-flex justify-center text-sm font-medium ">
                    ...
                    <ChevronDownIcon
                      aria-hidden="true"
                      className="-mr-1 ml-1 h-5 w-5 flex-shrink-0   -400 group-hover:  -500"
                    />
                  </MenuButton>
                </div>
                      ...
                      <MenuItem key={option.name}>
                        <a
                          ...
                          className={classNames(
                            option.current ? 'font-medium   -900' : '  -500',
              ...
              <button type="button" onClick={handleChangeShape} className="-m-2 ml-5 p-2   -400 hover:  -500 sm:ml-7">
              ...
              <button
                ...
                className="-m-2 ml-4 p-2   -400 hover:  -500 sm:ml-6 lg:hidden"
              >
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
              ...
              <form className="hidden lg:block">
                <h3 className="sr-only">Categories</h3>
                <ul role="list" className="space-y-4 border-b border-gray-200 pb-6 text-sm font-medium -900">
                  {subCategories.map((category) => (
                    <li key={category.name}>
                        ...
                        {section.options.map((option, optionIdx) => (
                          <div key={option.value} className="flex items-center">
                            <input
                              ...
                              className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                            />
                            <label htmlFor={`filter-${section.id}-${optionIdx}`} className="ml-3 text-sm   -600">
                              {option.label}
                            </label>
                          </div>
                        ))}
              ...
              <div className="lg:col-span-3">
                ...
                <div className={`mt-6 grid grid-cols-1 gap-x-6 gap-y-10 ${hasActive ? 'sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8' : 'sm:grid-cols-1 lg:grid-cols-2 xl:gap-x-4'}`}>
                  {
                    data.map(data => {
